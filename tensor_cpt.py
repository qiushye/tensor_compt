    ...
    if size_:
        ...
        for i in range(p):
            for j in range(q):
                for k in range(r):
                    if ori_data[i,j,k] == -1:
                        ori_data[i,j,k] = np.mean(ori_data[:,j,:])
...
def pre_impute(sparse_data,miss_loc,W,bias_bool = False):
    ...
    while ite < 200:
        ...
        for i in range(sp[0]):
            ...
            #print(np.sum(W[i,:,:]*(sparse_data-impute_data)[i,:,:]))
        for j in range(sp[1]):
            b1[1][j] = (1-seta*miu)*b[1][j]+seta*np.sum(W[:,j,:]*(sparse_data-impute_data)[:,j,:])
            #print(np.sum(W[:,j,:]*(sparse_data-impute_data)[:,j,:]))
        for k in range(sp[2]):
            b1[2][k] = (1-seta*miu)*b[2][i]+seta*np.sum(W[:,:,k]*(sparse_data-impute_data)[:,:,k])
...
def rmse_mape_rse(est_data,ori_data,miss_loc):
    ...
    mape = float(np.sum(np.fabs(diff_data)/ori_data)*100/len(miss_loc))
    rse = float(np.sum(diff_data**2)**0.5/np.sum(ori_data**2)**0.5)
    mae = float(np.sum(np.abs(est_data-ori_data))/len(miss_loc))
...
def compare_mr(ori_speeddata):
    ...
    for i in range(8):
        ...
        W = miss_data>0
        miss_data = pre_impute(miss_data,miss_pos,W,True)
    ...
    ax.legend(loc='best')
    plt.savefig(img_dir+'compare_mr_pre_rmse.png')
    plt.close()
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
...
if __name__ == '__main__':
    ...
    #sys.exit()
    RMSE_pre,MAPE_pre,RSE_pre,MAE_pre = rmse_mape_rse(miss_data,ori_speeddata,miss_pos)
    print('RMSE_pre,MAE_pre:',RMSE_pre,MAE_pre)
    for i in range(3):
        U,sigma,VT = np.linalg.svd(dtensor(miss_data).unfold(i))
        for r in range(len(sigma)):
            if sum(sigma[:r])/sum(sigma) > 0.9:
    ...
    est_tucker = tucker_cpt(miss_data,miss_pos,rank_set,W)
    RMSE_tk,MAPE_tk,RSE_tk,MAE_tk = rmse_mape_rse(est_tucker,ori_speeddata,miss_pos)
    print('RMSE_tk,MAE_tk',RMSE_tk,MAE_tk)
    est_cp = cp_cpt(miss_data,miss_pos,15)
    RMSE_cp,MAPE_cp,RSE_cp,MAE_cp = rmse_mape_rse(est_cp,ori_speeddata,miss_pos)
    print('RMSE_cp,MAE_cp',RMSE_cp,MAE_cp)
    alpha = [1/3,1/3,1/3]
    beta = [0.1,0.1,0.1]
    beta1 = beta.copy()
    gama = [2,2,2]
